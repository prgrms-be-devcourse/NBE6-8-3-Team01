    ...
    fun getRentRequestDetail(notificationId: Long, user: User): Map<String, Any?> {
        ...
        if (requester != null) {
            ...
            if (requesterRentLists.isNotEmpty()) {
                ...
                // Lombok getter methods are not recognized in Kotlin, using reflection or alternative approach
                ...
                // TODO: loanDate and returnDate access needs Java interop fix
                detail["loanDate"] = null // latestRentList.loanDate 
                detail["returnDate"] = null // latestRentList.returnDate

                // 🆕 처리 상태 확인 - using reflection to get status
                val status = try {
                    val statusField = latestRentList.javaClass.getDeclaredField("status")
                    statusField.isAccessible = true
                    statusField.get(latestRentList) as RentRequestStatus
                } catch (e: Exception) {
                    log.error("Failed to access status field: {}", e.message)
